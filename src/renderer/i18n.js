...
;(function () {
  const en = {
    ...
    'Показаны первые 10 000 элементов. Открой нужный путь вручную.': 'Showing the first 10,000 entries. Open the required path manually.',
    'Скачать папку': 'Download folder',
    'Скачать': 'Download',
    'Сохранить': 'Save',
    'Подтверждение': 'Confirmation',
    'Безвозвратно удалить настройки, серверы, известные SSH-ключи хостов, резервные копии, логи и дампы? Резервная копия не создаётся.': 'Permanently delete settings, servers, known SSH host keys, backups, logs, and crash dumps? No backup will be created.',
    'Да': 'Yes',
    'Открыть': 'Open',
    'Редактировать': 'Edit',
    ...
    'Показывать панель быстрых команд': 'Show quick-command bar',
    'Скрывает строку «Команда / Библиотека» над терминалом. Сами сохранённые команды не удаляются.': 'Hides the Command / Library row above the terminal. Saved commands are not deleted.',
    'Кнопки справа от вкладок': 'Buttons to the right of tabs',
    'Перетаскивайте строки или используйте стрелки, чтобы изменить порядок. Любую кнопку можно скрыть.': 'Drag the rows or use the arrows to change their order. Any button can be hidden.',
    'Показывать': 'Show',
    'Разделение терминала': 'Terminal split',
    'Перетащите для изменения порядка': 'Drag to change order',
    'Переместить влево': 'Move left',
    'Переместить вправо': 'Move right',
    'Зажмите и перетащите вкладку влево или вправо': 'Hold and drag the tab left or right',
    'Автоматическая проверка обновлений включена': 'Automatic update checks are enabled',
    'Проверить': 'Check',
    'Скрыть уведомление': 'Dismiss notification',
    'Установщик скачивается только с GitHub Releases и проверяется по SHA-512 перед запуском. Установка выполняется только после подтверждения.': 'The installer is downloaded only from GitHub Releases and verified with SHA-512 before launch. Installation starts only after confirmation.',
    ...
    'Плавная анимация текста при вводе и выводе': 'Smooth text animation for input and output',
    'История прокрутки (строк)': 'Scrollback history (lines)',
    'Скорость прокрутки колёсиком': 'Mouse-wheel scroll speed',
    'Колёсико в полноэкранных программах': 'Mouse wheel in full-screen programs',
    'PageUp/PageDown (рекомендуется для Claude/Codex)': 'PageUp/PageDown (recommended for Claude/Codex)',
    'По одной стрелке (совместимость)': 'One arrow at a time (compatibility)',
    'Стрелками пачкой (режим xterm)': 'Arrow-key batch (xterm mode)',
    'Отключить': 'Disabled',
